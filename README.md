...
lowest performance decrease (Drop range 0%-13%), better than zero-shot CoT strategy
...
Handles Molmo 2B model evaluation and CLIPScore caption evaluation metric.
...
4. **Zero-Shot Chain-of-Thought (CoT):** Model performs step-by-step reasoning before classification

**Key Findings:**
- Few-shot ICL shows best shortcut resistance (0%-13% performance drop)
- Zero-shot CoT shows moderate resistance (9%-32% performance drop)
...
|-------|-----------|-------------|--------|
| Molmo 2B | 2B | Environment 1 | `molmo.sh` / `src/evaluate_molmo2.py` |
| Qwen-VL | 8B | Environment 2 | `qwen.sh` / `src/evaluate_other_models.py` |
| Gemma-2B Vision | 4B | Environment 2 | `gemma.sh` / `src/evaluate_other_models.py` |
...
- `--strategy`: Prompting strategy (zero_shot / few_shot / cot / caption)
